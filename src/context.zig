...
pub const WindowFlags = struct {
    ...
    pub fn calc(self: @This()) u64 {
        ...
        if (self.highpixeldensity) {
            ret |= 0x2000;
        }
        if (self.mousecapture) {
            ret |= 0x4000;
        }
        if (self.mouserelativemode) {
            ret |= 0x8000;
        }
        if (self.alwaysontop) {
            ret |= 0x10000;
...
pub const ShaderFormat = struct {
    ...
    pub fn getFormat(self: @This()) !c.SDL_GPUShaderFormat {
        ...
        if (self.spirv) {
            return c.SDL_GPU_SHADERFORMAT_SPIRV;
        }
        if (self.dxbc) {
            return error.UnSupport;
        }
        if (self.dxil) {
            return c.SDL_GPU_SHADERFORMAT_DXIL;
        }
        if (self.msl) {
...
pub const Context = struct {
    ...
    swapchainTexture: *c.SDL_GPUTexture,
    pipelines: std.StringHashMap(*c.SDL_GPUGraphicsPipeline),

    pub fn initPtr(allocator: Allocator, api: ?[:0]const u8, title: [:0]const u8, width: u32, height: u32, flags: WindowFlags, shader_format: ShaderFormat) !*Self {
        var ret = try allocator.create(Self);
        ret.allocator = allocator;
        ret.shaderFormat = shader_format;
        ret.pipelines = std.StringHashMap(*c.SDL_GPUGraphicsPipeline).init(allocator);
        ...
        const gpup = c.SDL_CreateGPUDevice(
            @intCast(shader_format.calc()),
            true,
    ...
    pub fn deinitPtr(self: *Self) void {
        ...
        while (iter.next()) |entry| {
            ...
            c.SDL_ReleaseGPUGraphicsPipeline(self.gpu, entry.value_ptr.*);
        }
    ...
    pub fn createGPipeline(
        ...
        if (ppipeline) |pipeline| {
            c.SDL_ReleaseGPUShader(self.gpu, vshader);
            c.SDL_ReleaseGPUShader(self.gpu, fshader);
            const key = try std.fmt.allocPrint(self.allocator, "{s}", .{name});
    ...
    pub fn loadShader(
        ...
        if (pcode) |code| {
            ...
            const shader_info: c.SDL_GPUShaderCreateInfo = .{ .code = @as([*]const u8, @ptrCast(code))[0..code_size :0], .code_size = @intCast(code_size), .entrypoint = "main", .format = format, .stage = switch (shader_stage) {
                .Vert => c.SDL_GPU_SHADERSTAGE_VERTEX,
                .Frag => c.SDL_GPU_SHADERSTAGE_FRAGMENT,
            }, .num_samplers = @intCast(sample_count), .num_uniform_buffers = @intCast(ubuffer_count), .num_storage_buffers = @intCast(storage_buffer_count), .num_storage_textures = @intCast(storage_texture_count) };
    ...
    pub fn acquireCmd(self: *Self) !void {
    ...
    pub fn acquireSwapchain(self: *Self) !void {
        var swapchain: *c.SDL_GPUTexture = undefined;
        var w: u32 = undefined;
        var h: u32 = undefined;
        if (c.SDL_WaitAndAcquireGPUSwapchainTexture(self.cmd, self.window, @ptrCast(&swapchain), @ptrCast(&w), @ptrCast(&h))) {
            self.swapchainTexture = swapchain;
        } else {
            std.log.err("{s}", .{c.SDL_GetError()});
    ...
    pub fn beginRenderPass(self: *Self) !void {
        ...
        const prender_pass = c.SDL_BeginGPURenderPass(self.cmd, &target_info, 1, null);
    ...
    pub fn endRenderPass(self: *Self) void {
        c.SDL_EndGPURenderPass(self.renderPass);
    }

    pub fn bindGPipelineByPipeline(
        self: *Context,
        pipeline: *c.SDL_GPUGraphicsPipeline,
    ) void {
        c.SDL_BindGPUGraphicsPipeline(self.renderPass, pipeline);
    }
    ...
    pub fn bindGPipeline(
        ...
        const p = self.pipelines.get(name);
        if (p) |pipeline| {
            c.SDL_BindGPUGraphicsPipeline(self.renderPass, pipeline);
        } else {
            return error.FailedToGetPipeline;
        }
    }

    pub fn setViewport(self: *Self) !void {
        var w: i32 = undefined;
        var h: i32 = undefined;

        if (!c.SDL_GetWindowSize(self.window, @ptrCast(&w), @ptrCast(&h))) {
        ...
        c.SDL_SetGPUViewport(self.renderPass, &vp);
    }

    pub fn drawPrimitives(
        self: *Self,
    ) void {
        c.SDL_DrawGPUPrimitives(self.renderPass, 3, 1, 0, 0);
    }

    pub fn submitCmd(
        self: *Self,
    ) void {
        if (!c.SDL_SubmitGPUCommandBuffer(self.cmd)) {
